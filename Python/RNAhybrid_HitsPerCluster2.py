...
targets = open("C:/RNAseq/mir_targets/rnahybrid/targets_f2-7_e10p01_k4-2.srt.txt", "r")

first = targets.readline()
mRNA = first[first.find("ENSCINT"):].split("\t")[0]
...
for line in targets:
    ...
    if mir == line[line.find("cin"):].split("\t")[0]: #check if miR is same as current (previous line)
        ...
        if mRNA == line[line.find("ENSCINT"):].split("\t")[0]: #check if mRNA is same as current (previous line)
            ...
            unique += 1
            mRNA = line[line.find("ENSCINT"):].split("\t")[0] # reset current mRNA
    else:
        #print mir, mRNA, count # Final mRNA in a cluster, hits
        print mir, cluster, unique # miR hits/cluster & unique targets/cluster
        mRNA = line[line.find("ENSCINT"):].split("\t")[0] # reset current mRNA for new cluster
