...
targets = open("C:/RNAseq/mir_targets/rnahybrid/targets_6bp_e20p01_k1-3.srt.txt", "r")

first = targets.readline()
mRNA = str(first.split()[0])
mir = str(first.split()[2])
cluster = 0
unique = 1

for line in targets:
    if mRNA == str(line.split()[0]): #check if mRNA is same as previous
        cluster += 1 # increment cluster size for each target in an mRNA
        if mir == str(line.split()[2]): #check if miR is same as previous
            pass
        else: #if new miR
            unique += 1 #increment unique miRs for current mRNA
            mir = str(line.split()[2]) # reset current miR
    else:
        ...
        mRNA = str(line.split()[0]) # reset current mRNA for new cluster
        mir = str(line.split()[2]) # reset current miR for new cluster
